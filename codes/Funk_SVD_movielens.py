# -*- coding: utf-8 -*-
"""
Created on Sat Mar 20 21:12:24 2021
...
def compute_Funk_SVD (R, K, alpha, beta, nb_iterations):
    ...
    b = np.mean(R[np.where(R != 0)]) #moyenne des rij pour les rij != 99 (cases non vides de R)
    ...
    def total_error(): 
        """
        fonction qui calcule la l'erreur totale
        """
        indice_lig, indice_col = np.where(R != 0)  # indice_lig : indices des lignes où rij != 0
                                                   # indice_col : indices des colonnes où rij != 0
